...
| BIRTHS (T2D biomarker) | Control=59.9, Pre-DM=60.7, T2D=66.7 (Kruskal-Wallis p=0.008) — modest but statistically significant, not a three-stage separation |
...
AD shows the stronger signal here, plausibly because its sample size (n=706) is
~3.7× larger than T2D's (n=189), not because AD blood biology is intrinsically more
informative than T2D.

---

## 📦 Data Sources

| Dataset | Disease | Type | n available (public supplementary files) | Source |
|---|---|---|---|---|
...
│   ├── ml_classifier.py             #   5 classifiers, leakage-corrected
...
# 1. Download GEO data (curl-based; ~300MB total: GSE221521, GSE63060, GSE63061
#    + GPL6947/GPL10558 annotation)
...
Result: Control=41.1±16.4, MCI=39.0±16.1, AD=40.2±16.9 (n=706)
...
Result: Control=59.9±19.2, Pre-DM=60.7±12.0, T2D=66.7±14.7 (n=189)
Kruskal-Wallis H=9.67, p=0.0079 → statistically significant, but modest effect.
```
...
## 📊 ML Model Performance (leakage-corrected)
...
derived from the DEG results — and since T2D has zero significant DEGs, there is
no data-derived T2D signature to score drugs against. As implemented, every drug
scores 1.000 for both diseases, which is a methodological artefact, not a ranked
empirical finding. This needs restructuring before any drug ranking is reported as
a result.
